...
fn load_resources_from_file(ca: &mut ResMut<CommonAssets>,
                            asset_server: &Res<AssetServer>,
                            g: &Res<GraphDefinitionRes>) {
  for node in &g.graph_defn.nodes {
    match &node.attrs {
      Some(attrs) => {
        if attrs.icon.is_some() {
          let icon_string = attrs.icon.clone().unwrap();
          ca.resource_map.insert(icon_string.clone(),
